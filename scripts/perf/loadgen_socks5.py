...
async def read_exact(reader: asyncio.StreamReader, n: int, timeout_s: float) -> bytes:
...
def build_socks5_connect(host: str, port: int) -> bytes:
    try:
        ...
        if isinstance(ip, ipaddress.IPv4Address):
            ...
            atyp = 0x04
            addr = ip.packed
        return b"\x05\x01\x00" + bytes([atyp]) + addr + port.to_bytes(2, "big")
    except ValueError:
        host_bytes = host.encode("utf-8")
        if len(host_bytes) > 255:
            raise ValueError("host too long for SOCKS5 domain")
        return (
            b"\x05\x01\x00"
            + b"\x03"
...
async def socks5_handshake_and_connect(
    reader: asyncio.StreamReader,
    ...
    await writer.drain()
    resp = await read_exact(reader, 2, timeout_s=timeout_s)
    if resp[0] != 0x05 or resp[1] != 0x00:
        raise RuntimeError(f"socks5 method rejected: {resp!r}")

    req = build_socks5_connect(host, port)
    writer.write(req)
    await writer.drain()

    head = await read_exact(reader, 4, timeout_s=timeout_s)
    ver, rep, _rsv, atyp = head
    if ver != 0x05:
        raise RuntimeError(f"socks5 bad version: {ver}")
    if rep != 0x00:
        raise RuntimeError(f"socks5 connect failed rep={rep}")

    if atyp == 0x01:
        await read_exact(reader, 4, timeout_s=timeout_s)
    elif atyp == 0x03:
        ln = (await read_exact(reader, 1, timeout_s=timeout_s))[0]
        await read_exact(reader, ln, timeout_s=timeout_s)
    elif atyp == 0x04:
        await read_exact(reader, 16, timeout_s=timeout_s)
    else:
        raise RuntimeError(f"socks5 unknown atyp={atyp}")

    await read_exact(reader, 2, timeout_s=timeout_s)
...
async def one_request(
    ...
    try:
        ...
        await socks5_handshake_and_connect(
            reader=reader,
            writer=writer,
            host=target_host,
            port=target_port,
        ...
        code = await read_http_message(reader, timeout_s=timeout_s)
...
def main():
    ...
    ap.add_argument("--url", required=True, help="http://host:port/path (requested via SOCKS5 CONNECT)")
